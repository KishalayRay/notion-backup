...
import { TheNewsApiContext } from "./TheNewsApiContext/listContext";

import { GetNews, CreateNews } from "./TheNewsApiContext/apiCalls";
...
const TheNewsApi = () => {
  ...
  const { dispatch, news } = useContext(TheNewsApiContext);
  const [code, setCode] = useState("");
  const [load, setLoad] = useState(false);
  useEffect(() => {
    GetNews(axiosPrivate, dispatch);
  }, [dispatch]);
  const handleSubmit = async (e) => {
    ...
    await CreateNews(smallCode, axiosPrivate, dispatch);
    setLoad(false);
  };
